...
NO REDACTES PREGUNTAS. Construí solamente un plan.

Descubrí al menos ${Math.max(count * 3, 20)} candidatos cuando la fuente lo permita. Para cada candidato indicá objective, centralFact, perspectiveKey, cluster, subtopic, category y sourceBasis.
centralFact = la información central que el estudiante debe recuperar para responder.

Luego seleccioná EXACTAMENTE ${count} objetivos si existen suficientes contenidos defendibles.
IMPORTANTE:
- Un mismo tema puede tener varias preguntas.
- Cambiar el formato o la redacción NO crea un nuevo objetivo.
- Dos candidatos son duplicados si la respuesta central que debe recuperar el estudiante es esencialmente la misma.
- Una perspectiva diferente es válida solo cuando cambia la información central evaluada.
- Color, posición o una etiqueta repetida del esquema no son contenido académico nuevo salvo que la fuente les otorgue significado explícito.
- Una pregunta debe tener un solo objetivo principal.
- No uses conocimiento externo.

Antes de devolver el plan verificá que cada centralFact sea distinto de los demás y esté respaldado por sourceBasis.
Si hay ${count} contenidos centrales distintos respaldados, devolvé ${count}. Solo devolvé menos si realmente no existen suficientes en todo el material seleccionado.
